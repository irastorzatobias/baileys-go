...
func initEnvConfig() {
	...
	if envAPIEndpoint := viper.GetString("whatsapp_api_endpoint"); envAPIEndpoint != "" {
		config.WhatsappAPIEndpoint = envAPIEndpoint
	}
	if envTwilioAccountSID := viper.GetString("twilio_account_sid"); envTwilioAccountSID != "" {
		config.TwilioAccountSID = envTwilioAccountSID
	}
	if envCompanyNID := viper.GetInt("company_nid"); envCompanyNID != 0 {
		config.CompanyNID = envCompanyNID
	}
...
func initFlags() {
	...
	rootCmd.PersistentFlags().StringVarP(
		&config.WhatsappAPIEndpoint,
		"whatsapp-api-endpoint", "",
		config.WhatsappAPIEndpoint,
		`base URL to forward inbound messages in Twilio-compatible format --whatsapp-api-endpoint <string> | example: --whatsapp-api-endpoint="https://example.com"`,
	)
	rootCmd.PersistentFlags().StringVarP(
		&config.TwilioAccountSID,
		"twilio-account-sid", "",
		config.TwilioAccountSID,
		`Twilio Account SID included in forwarded payloads --twilio-account-sid <string> | example: --twilio-account-sid="ACXXXX"`,
	)
	rootCmd.PersistentFlags().IntVarP(
		&config.CompanyNID,
		"company-nid", "",
		config.CompanyNID,
		`company NID included in outbound device callbacks --company-nid <int>`,
	)
	rootCmd.PersistentFlags().BoolVarP(
