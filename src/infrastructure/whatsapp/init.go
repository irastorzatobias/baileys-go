...
func handleMessage(ctx context.Context, evt *events.Message, chatStorageRepo domainChatStorage.IChatStorageRepository) {
	...
	// Forward outbound (from-me) messages to external callback
	if evt.Info.IsFromMe {
		go func() {
			if err := forwardOutboundDeviceMessage(ctx, evt, chatStorageRepo); err != nil {
				log.Errorf("Failed to forward outbound device message: %v", err)
			}
		}()
	}

	go func() {
		if err := forwardTwilioCallback(ctx, evt, chatStorageRepo); err != nil {
			log.Errorf("Failed to forward inbound message to Twilio callback: %v", err)
		}
	}()
}
